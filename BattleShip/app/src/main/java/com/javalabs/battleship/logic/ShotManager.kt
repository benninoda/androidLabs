package com.javalabs.battleship.logic

import android.util.Log
import com.javalabs.battleship.*
...
import com.javalabs.battleship.battle_field.BaseBattleField
...
class ShotManager {
    ...
    fun markEdgeCells(cells: MutableList<Coordinate>) {
        ...
        when {
            ...
            cells[0].x == cells[1].x -> {
                val max = getMaxCoordinate(cells, Direction.HORIZONTAL)
                val min = getMinCoordinate(cells, Direction.HORIZONTAL)
            ...
            cells[0].y == cells[1].y -> {
                val max = getMaxCoordinate(cells, Direction.VERTICAL)
                val min = getMinCoordinate(cells, Direction.VERTICAL)
    ...
    fun getMaxCoordinate(list: MutableList<Coordinate>, orientation: Direction): Coordinate {
        return if (orientation == Direction.VERTICAL) {
            list.maxBy { it.x }!!
        } else {
            list.maxBy { it.y }!!
        }
    ...
    fun getMinCoordinate(list: MutableList<Coordinate>, orientation: Direction): Coordinate {
        return if (orientation == Direction.VERTICAL) {
            list.minBy { it.x }!!
        } else {
            list.minBy { it.y }!!
    ...
    fun markHorizontalNeighbours(cell: Cell) {
        Log.e("D", "mark horizontal neighbours")
    ...
    fun markVerticalNeighbours(cell: Cell) {
        Log.e("D", "in mark vertical neighbours")
